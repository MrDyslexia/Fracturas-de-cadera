...
const { recalcularIndicadores, recalcularIndicadoresEpisodio } = require('../services/riesgoRefracturaService');
    ...
    for (const episodio of episodios) {
      ...
      if (resultado.controles && resultado.controles.length > 0) {
        resultado.controles.forEach((control, idx) => {
          ...
          else if (nivel === 'MODERADO') nivelColor = '\x1b[33m'; // amarillo
          ...
          if (control.alerta) {
            console.log(`    🚨 Alerta generada: ${control.alerta.severidad}`);
          }
          
          if (control.indicadores && control.indicadores.length > 0) {
            const factoresCumplen = control.indicadores.filter(i => i.valor > 0);
            console.log(`    Factores que cumplen: ${factoresCumplen.length}/${control.indicadores.length}`);
            ...
            if (factoresCumplen.length > 0) {
              ...
              factoresCumplen.forEach(factor => {
                ...
                console.log(`      - ${factor.tipo}: +${factor.valor} puntos`);
                if (detalles.mensaje) {
                  console.log(`        ${detalles.mensaje.substring(0, 60)}...`);
                }
              });
            }
          }

...
const { recalcularIndicadores } = require('../services/riesgoRefracturaService');
...
function parseArgs() {
  ...
  if (CONFIG.episodioId && (CONFIG.limit || CONFIG.skip)) {
    logWarning('Las opciones --limit y --skip se ignoran cuando se usa --episodio');
    CONFIG.limit = null;
    CONFIG.skip = null;
  }

  if (CONFIG.verbose && CONFIG.quiet) {
    logWarning('No se puede usar --verbose y --quiet al mismo tiempo. Se usará --verbose');
    CONFIG.quiet = false;
  }
}
...
async function procesarEpisodio(episodioId) {
  try {
    ...
    if (CONFIG.dryRun) {
      logVerbose(`  [DRY-RUN] Episodio ${episodioId} ${tieneControles ? `con ${controles.length} control(es)` : 'sin controles'}`);
    ...
    for (const controlResult of controlesResultados) {
      ...
      if (CONFIG.verbose) {
        ...
        logVerbose(`  Control ${controlId}: Nivel ${nivel}, Puntaje ${puntaje}`);
      }
    }
    ...
    else if (nivelMasAlto === 'MODERADO') stats.episodiosConRiesgoModerado++;
    else if (nivelMasAlto === 'ALTO') stats.episodiosConRiesgoAlto++;

    stats.alertasGeneradas += alertasEnEsteEpisodio;

    logVerbose(`  ${COLORS.green}✓ Episodio ${episodioId} procesado. Nivel: ${nivelMasAlto}, Alertas: ${alertasEnEsteEpisodio}${COLORS.reset}`);
...
async function procesarEpisodiosEnLotes(episodios) {
  ...
  logInfo(`Procesando ${total} episodio(s) en lotes de ${CONFIG.batchSize}...`);
  ...
  for (let i = 0; i < total; i += CONFIG.batchSize) {
    ...
    if (!CONFIG.quiet && !CONFIG.verbose) {
      process.stdout.write(`\r${COLORS.cyan}Progreso: ${procesados}/${total} (${porcentaje}%)${COLORS.reset}`);
    }
  }
...
async function obtenerEpisodios() {
  const whereClause = {};
  const options = { attributes: ['episodio_id'], order: [['episodio_id', 'ASC']] };
...
function mostrarResumen() {
  ...
  const promedioTiempo = stats.totalEpisodios > 0 
  ...
  console.log(`  ${COLORS.green}Riesgo BAJO:${COLORS.reset}       ${stats.episodiosConRiesgoBajo}`);
  console.log(`  ${COLORS.yellow}Riesgo MODERADO:${COLORS.reset}   ${stats.episodiosConRiesgoModerado}`);
  console.log(`  ${COLORS.red}Riesgo ALTO:${COLORS.reset}       ${stats.episodiosConRiesgoAlto}`);
  ...
  if (stats.errores > 0) {
    ...
    if (CONFIG.verbose) {
      ...
      if (stats.erroresDetalle.length > 10) {
        console.log(`\n  ... y ${stats.erroresDetalle.length - 10} error(es) más`);
      }
    }
  }
  ...
  if (CONFIG.dryRun) {
    logWarning('MODO DRY-RUN: No se realizaron cambios en la base de datos');
  }

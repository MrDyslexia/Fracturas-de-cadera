...
function sortByDateAsc<T extends { fecha_recepcion?: string | null; fecha?: string | null; fecha_toma?: string | null }>(arr: T[]) {
  return [...arr].sort((a, b) => {
...
export async function generarMinutaPDF(
  ...
  autor: AutorMinuta,
  blocks: MinutaBlocks = {},
  opts?: { maxExamenes?: number } // default 5
) {
  if (!p?.general) {
  ...
  const edadDesc = g?.edad != null ? `${g.edad} años${g?.edad_meses ? ` ${g.edad_meses} meses` : ""}` : "—";
  const sexo = g?.sexo ?? "—";
  ...
  type Valor = { fecha: string; valor: number | undefined; unidad?: string | undefined };
  ...
  muestrasOrdenadas.forEach((m) => {
    ...
    (m?.resultados ?? []).forEach((r: any) => {
      ...
      if (series[key]) {
        ...
        series[key].push({ fecha: f, valor: isNaN(Number(n)) ? undefined : n, unidad: r?.unidad });
      }
    });
  });
  ...
  const resumenBody = [
    ...
    [
      "Profesional",
      [autor?.nombre ?? "—", [autor?.cargo, autor?.rut, autor?.unidad].filter(Boolean).join(" • ")].filter(Boolean).join(" — "),
    ],
  ];
  ...
  (autoTable as AutoTableFn)(doc, {
    ...
    body: [
      ["Hemoglobina", ultimos.HB.valor ?? "—", ultimos.HB.unidad ?? "g/dL", safeDate(ultimos.HB.fecha)],
      ["Glucosa", ultimos.GLUCOSA.valor ?? "—", ultimos.GLUCOSA.unidad ?? "mg/dL", safeDate(ultimos.GLUCOSA.fecha)],
      ["Colesterol Total", ultimos.COLESTEROL_TOTAL.valor ?? "—", ultimos.COLESTEROL_TOTAL.unidad ?? "mg/dL", safeDate(ultimos.COLESTEROL_TOTAL.fecha)],
      ["Triglicéridos", ultimos.TRIGLICERIDOS.valor ?? "—", ultimos.TRIGLICERIDOS.unidad ?? "mg/dL", safeDate(ultimos.TRIGLICERIDOS.fecha)],
    ],
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  doc.text(`Exámenes recientes (últimos ${Math.min(maxExamenes, muestrasOrdenadas.length)})`, margin, y);
  y += 12;

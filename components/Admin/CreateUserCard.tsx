...
export function CreateUserCard() {
  ...
  const puedeCrear = useMemo(() => {
    ...
    const requiereRP = cargo !== 'FUNCIONARIO'; // nos pediste solicitar ambos RUT
    ...
    <section className="bg-white rounded-2xl shadow-sm border border-blue-100 overflow-hidden">
      ...
      <form onSubmit={onSubmit} className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="col-span-1 md:col-span-2">
          <h3 className="text-sm font-semibold text-blue-800">Datos de identidad</h3>
        </div>

        <input className="input" placeholder="RUT nacional (12.345.678-9)"
        ...
        <div className="flex gap-3">
          <select className="input" value={sexo} onChange={e=>setSexo(e.target.value as any)}>
        ...
        <input className="input" placeholder="Contraseña (min. 8, 1 mayúscula, 1 número)"
               value={password} onChange={e=>setPassword(e.target.value)} type="password" />

        {/* Profesional */}
        <div className="col-span-1 md:col-span-2 mt-2">
          <h3 className="text-sm font-semibold text-blue-800">Perfil profesional</h3>
        </div>

        <select className="input" value={cargo} onChange={e=>setCargo(e.target.value as any)}>
        ...
        <input className="input" placeholder="RUT profesional (12.345.678-9)"
               value={rutProfesional} onChange={e=>setRutProfesional(e.target.value)} />

        <input className="input" placeholder="Especialidad (opcional)"
               value={especialidad} onChange={e=>setEspecialidad(e.target.value)} />
        <input className="input" placeholder="Hospital (opcional)"
               value={hospital} onChange={e=>setHospital(e.target.value)} />
        <input className="input" placeholder="Departamento (opcional)"
        ...
        <div className="col-span-1 md:col-span-2 flex items-center justify-end gap-3 pt-2">
          <button type="submit" disabled={!puedeCrear || loading}

...
(async () => {
  try {
    ...
    let admin = await Usuario.findOne({ where: { [Op.or]: [{ correo: adminEmail }, { rut: adminRut }] } });
    let created = false;
    if (!admin) {
    ...
    console.log(created ? `✅ Usuario administrador creado: ${adminEmail}` : `ℹ️ Usuario administrador ya existe: ${adminEmail}`);

...
async function list(req, res) {
  try {
    ...
    const where = {};
    if (!isEmpty(req.query.especialidad)) where.especialidad = String(req.query.especialidad).trim();
...
async function create(req, res) {
  try {
    ...
    if (exists) return res.status(409).json({ error: 'El usuario ya tiene perfil de Tecnólogo' });
    ...
    // Auditoría
    await logRegistro(req, 'TECNOLOGO_CREADO');
...
async function update(req, res) {
  try {
    ...
    if (rut_profesional !== undefined) row.rut_profesional = normStr(rut_profesional);
    if (especialidad !== undefined) row.especialidad = normStr(especialidad);

    await row.save();

    // Auditoría
    await logRegistro(req, 'TECNOLOGO_ACTUALIZADO');
...
async function remove(req, res) {
  try {
    ...
    // Auditoría
    await logRegistro(req, 'TECNOLOGO_ELIMINADO');

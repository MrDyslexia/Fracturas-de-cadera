...
async function create(req, res) {
  try {
    const { episodio_id, tipo_control, cambios ,profesional_id} = req.body || {};
    const {
      resumen,
      fecha_hora_control,
      comorbilidades,
    ...
    const { tabaco, alcohol, corticoides_cronicos, taco } = habitos || {};
    if (!episodio_id)
    ...
    if (antro) {
      ...
      (peso !== undefined && peso !== null && !Number.isNaN(Number(peso))) ||
      (altura !== undefined && altura !== null && !Number.isNaN(Number(altura)))
    ) {
    ...
    if (!existeControl && tipo_control === "Inicial") {
      ...
      if (tabaco !== undefined) epi.tabaco = tabaco === null ? null : !!tabaco;
      if (alcohol !== undefined)
        epi.alcohol = alcohol === null ? null : !!alcohol;
      if (corticoides_cronicos !== undefined)
        epi.corticoides_cronicos =
          corticoides_cronicos === null ? null : !!corticoides_cronicos;
    ...
    const pro = await models.ProfessionalProfile.findByPk(
      req.body.profesional_id
    );
    ...
    const cleanComor = normComor === null ? null : normComor.map(removeTildes);
    ...
    const created = await models.ControlClinico.create({
      ...
      corticoides_cronicos:
        corticoides_cronicos === undefined ? null : !!corticoides_cronicos,
    ...
    try {
      await riesgoService.recalcularIndicadoresControl(created.control_id);
    } catch (riskError) {
...
async function update(req, res) {
  try {
    ...
    const pro = await models.ProfessionalProfile.findByPk(body.profesional_id);
    const user = await models.User.findByPk(pro.user_id);
    const nombre =
      user.nombres + " " + user.apellido_paterno + " " + user.apellido_materno;
    if (nombre) row.profesional_nombre = nombre;

    if (body.origen !== undefined) row.origen = body.origen;
    if (body.tipo_control !== undefined) row.tipo_control = body.tipo_control;
    ...
    if (body.corticoides_cronicos !== undefined)
      row.corticoides_cronicos =
        body.corticoides_cronicos === null ? null : !!body.corticoides_cronicos;
    ...
    if (body.transfusion !== undefined)
      row.transfusion = body.transfusion === null ? null : !!body.transfusion;

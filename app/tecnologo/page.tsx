...
function CardHeader({ children }: { children: React.ReactNode }) { return <div className="p-6 pb-4">{children}</div>; }
function CardContent({ children }: { children: React.ReactNode }) { return <div className="px-6 pb-6">{children}</div>; }
function CardTitle({ children }: { children: React.ReactNode }) { return <h3 className="text-lg font-semibold text-gray-900">{children}</h3>; }
function CardDescription({ children }: { children: React.ReactNode }) { return <p className="text-sm text-gray-600 mt-1">{children}</p>; }
function CardHeaderWithIcon({ icon, title, subtitle }: { icon: React.ReactNode; title: string; subtitle: string }) {
  return (
    <CardHeader>
...
}: { children: React.ReactNode; onClick?: () => void; variant?: "default" | "secondary" | "ghost"; className?: string }) {
  const base = "inline-flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2";
  const variants = {
...
function LabExamsPanel() {
  ...
  const optsTipoMuestra = useMemo(
    () => ["", ...Array.from(new Set(examenes.flatMap((e) => e.muestras?.map((m) => m.tipo_muestra) ?? [])))],
    [examenes]
  );
  ...
  const examenesFiltrados = useMemo(() => {
    ...
    return examenes.filter((ex) => {
      const byExamen = !filtros.tipoExamen || ex.tipo_examen === filtros.tipoExamen;
          ...
          <div className="space-y-4">
            {examenesFiltrados.length === 0 ? (
              ...
              examenesFiltrados.map((ex) => {
                ...
                const totalResultados = ex.muestras?.reduce((acc, m) => acc + (m.Resultados?.length || 0), 0) || 0;
                const primeraFecha = ex.muestras?.[0]?.fecha_recepcion;

                return (
                  <div key={String(id)} className="border border-gray-200 rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow">
                    <div className="p-4 cursor-pointer" onClick={() => toggle(id)}>
                    ...
                    {abierto && (
                      <div className="border-t border-gray-200 bg-gray-50 p-4">
                        <div className="flex justify-between items-center mb-4">
                          ...
                          <Button onClick={() => descargarExamenCompleto(ex)}>
                            ...
                            <div key={String(m.muestra_id)} className="bg-white rounded-lg border border-gray-200 p-4">
                              ...
                              {m.Resultados?.length ? (
                                <div className="space-y-2">
                                  {m.Resultados.map((r) => (
                                    <div key={String(r.resultado_id)} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                                      <div className="flex items-center gap-4">
                                        <div className="flex flex-col">
                                ...
                                <div className="text-center py-2 text-gray-500 text-sm">No hay resultados disponibles para esta muestra</div>
                              )}
                            </div>
                          ))}
                        </div>
